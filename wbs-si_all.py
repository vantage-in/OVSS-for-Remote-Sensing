...
IMAGE_DIR = Path('data/WBS-SI/Images')
# Updated LABEL_DIR to the new path
LABEL_DIR = Path('data/WBS-SI/Masks_cvt')
BASE_OUTPUT_DIR = Path('present/wbs-si')
...
def run_inference(model_name, output_dir_name, image_files):
    ...
    if model_name == 'ours':
        model = ProxySegEarthSegmentationCatRandom(
            ...
            prob_thd=0.7, cls_variant="none", vfm_model="dino"
        )
