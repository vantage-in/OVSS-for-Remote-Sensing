...
IMAGE_DIR = Path('data/UDD/UDD/UDD5/val/src')
# Updated LABEL_DIR to the new path
LABEL_DIR = Path('data/UDD/UDD/UDD5/val/gt')
BASE_OUTPUT_DIR = Path('present/udd5')
...
class_info = {
    0: ("vegetation", "#4BB549"),               # 네이비
    1: ("building", "#DE1F07"),          # 청록 (Teal)
    2: ("road", "#FFFFFF"),    # 노랑
    3: ("vehicle", "#0045FF"),        # 연두 (GreenYellow)
    4: ("background", "#000000")    # 주황
}
...
def run_inference(model_name, output_dir_name, image_files):
    ...
    if model_name == 'ours':
        model = ProxySegEarthSegmentationCatRandom(
            ...
            prob_thd=0.4, bg_idx=4, cls_variant="none", vfm_model="dino"
        )
        ...
        model = SegEarthSegmentation(
            ...
            feature_up_cfg=dict(
                ...
                model_path='simfeatup_dev/weights/xclip_jbu_one_million_aid.ckpt'),
            cls_token_lambda=-0.3, name_path='./configs/my_name.txt',
            prob_thd=0.4, bg_idx=4, cls_variant="none"
        )
    model.to('cuda')
    
    # 3. Process all images
    # image_files = list(IMAGE_DIR.glob('*.tif')) + list(IMAGE_DIR.glob('*.png')) + list(IMAGE_DIR.glob('*.jpg') + list(IMAGE_DIR.glob('*.JPG'))
