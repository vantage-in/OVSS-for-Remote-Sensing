...
IMAGE_DIR = Path('data/vaihingen/img_dir/val')
# Updated LABEL_DIR to the new path
LABEL_DIR = Path('data/vaihingen/ann_dir/val')
BASE_OUTPUT_DIR = Path('present/vaihingen')
...
def run_inference(model_name, output_dir_name, image_files):
    ...
    if model_name == 'ours':
        model = ProxySegEarthSegmentationCatRandom(
            ...
            prob_thd=0.1, bg_idx=5, cls_variant="none", vfm_model="dino"
        )
        ...
        model = SegEarthSegmentation(
            ...
            feature_up_cfg=dict(
                ...
                model_path='simfeatup_dev/weights/xclip_jbu_one_million_aid.ckpt'),
            cls_token_lambda=-0.3, name_path='./configs/my_name.txt',
            prob_thd=0.1, bg_idx=5, cls_variant="none"
        )
    model.to('cuda')
...
name_list = ['impervious surface', 'building', 'low vegetation', 'tree', 'car', 'clutter']
