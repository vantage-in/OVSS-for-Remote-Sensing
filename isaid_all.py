...
IMAGE_DIR = Path('data/iSAID/img_dir/val')
# Updated LABEL_DIR to the new path
LABEL_DIR = Path('data/iSAID/ann_dir/val')
BASE_OUTPUT_DIR = Path('present/isaid')
...
def run_inference(model_name, output_dir_name, image_files):
    ...
    if model_name == 'ours':
        model = ProxySegEarthSegmentationCatRandom(
            ...
            prob_thd=0.4, cls_variant="none", vfm_model="dino"
        )
...
if PROCESS_LABELS:
    ...
    for img_path in sampled_image_files:
        base_name = img_path.stem
        found_labels = list(LABEL_DIR.glob(f"{base_name}_instance_color_RGB.*"))
