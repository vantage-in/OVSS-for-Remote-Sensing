...
IMAGE_DIR = Path('data/GlobalRoadSet_Val/Massachusetts_test_49/img')
# Updated LABEL_DIR to the new path
LABEL_DIR = Path('data/GlobalRoadSet_Val/Massachusetts_test_49/label_cvt')
BASE_OUTPUT_DIR = Path('present/massachusetts')
...
def run_inference(model_name, output_dir_name, image_files):
    ...
    if model_name == 'ours':
        model = ProxySegEarthSegmentationCatRandom(
            ...
            prob_thd=0.8, cls_variant="none", vfm_model="dino"
        )
